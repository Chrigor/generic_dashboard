...
export default function auth(
  ...
  requestParams.oauth_timestamp = new Date().getTime().toString().slice(0, 10); // we need just the first 10 digits from current time

  const encodedSignature = oauthSignature.generate(
    type,
    `${FLUIG_SERVER}/${endpoint}`,
    requestParams,
